...
static int set_base_name(char* filename, char* basename)
{
   unsigned int len = 0;
   char* p2c = filename;
   for ( ; *p2c != 0 ; p2c++) {
...
int main(int argc, char* argv[])
{
        ...
        if (singleFiles) {
                ...
                if (NULL == outputName) {
                        perror("malloc for output");
                        return -1;
                }
        }
        baseName = malloc(1024);
        if (NULL == baseName) {
           perror("mallocfor base");
           return -1;
        }
        iList.len = 0;
        iList.size = 4096;
        ...
        for (j=optind ; j < argc ; j++) {
                ...
                if (NULL == outFile) {
                        ...
                        set_base_name(outputName, baseName);
                        fprintf(outFile, "#ifndef %s_HEADER\n", baseName);
                        fprintf(outFile, "#define %s_HEADER\n", baseName);
                }
                ...
                if (singleFiles) {
                        fprintf(outFile, "#endif /* %s_HEADER */\n", baseName);
                        fclose(outFile);
                        outFile = NULL;
                }
        }
